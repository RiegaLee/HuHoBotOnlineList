...
import cn.huohuas001.huhobot.onlinelist.util.JsonStrings;
...
/**
 * 优先调用分支公开的事件图片方法；没有公开方法时，回退到 HuHoBot 随附 QQ SDK 的
 * file_data 上传端点。整个实现都在本附属 JAR 内。
 */
public final class ReflectiveImageReplyApi implements ImageReplyApi {
    private static final String[] QCLIENT_CLASSES = {
        "cn.huohuas001.bot.QClient"
    };
    ...
    public ImageReplyResult reply(ImageReplyRequest request) {
        if (request.getImageSize() == 0) {
            return ImageReplyResult.failure("validation", "image is empty", null);
        }
        if (request.getImageSize() > maxBytes) {
            return ImageReplyResult.failure(
                "validation",
                "image is larger than configured max-bytes: " + request.getImageSize(),
                null
            );
        }
        ...
        ImageReplyResult messageChainResult = tryQqMessageChain(request);
        if (messageChainResult != null && messageChainResult.isSuccess()) return messageChainResult;
    ...
    private ImageReplyResult tryQqMessageChain(ImageReplyRequest request) {
        ...
        try {
            ...
            Reflect.invoke(chain, "text", request.getText().isEmpty() ? "[在线列表]" : request.getText());
            Reflect.invoke(chain, "image", request.getImageBytes());
            Method send = Reflect.findCompatibleMethod(event.getClass(), "sendMessage", chain);
    ...
    private ImageReplyResult tryNativeEventApi(ImageReplyRequest request) {
        ...
        for (Object[] attempt : attempts) {
            ...
            try {
                ...
                return ImageReplyResult.failure("event-native:" + methodName, error.getMessage(), error);
            }
        }
        return null;
    }
    ...
    private ImageReplyResult tryQqSdk(ImageReplyRequest request) {
        try {
            ...
            Class<?> qClientClass = loadFirst(loader, QCLIENT_CLASSES);
            ...
            StringBuilder messageJson = new StringBuilder(256)
                .append("{\"content\":").append(JsonStrings.quote(request.getText().isEmpty() ? "[在线列表]" : request.getText()))
                .append(",\"msg_type\":7,\"media\":{\"file_info\":").append(JsonStrings.quote(fileInfo)).append('}');
            if (!request.getMessageId().isEmpty()) {
                messageJson.append(",\"msg_id\":").append(JsonStrings.quote(request.getMessageId()));
            }
            messageJson.append(",\"msg_seq\":").append(request.getMessageSequence()).append('}');
            Reflect.invoke(groupBase, "send", request.getGroupOpenId(), messageJson.toString(), headers);
    ...
    private static Map<?, ?> messageHeaders(ClassLoader loader) {
        try {
            ...
            // Older forks can rely on the HTTP client's declared JSON content type.
        }
    ...
    private static Class<?> loadFirst(ClassLoader loader, String[] names) throws ClassNotFoundException {
        ...
        throw last == null ? new ClassNotFoundException("HuHoBot QClient") : last;
    }
